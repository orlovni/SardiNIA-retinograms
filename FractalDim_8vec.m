%
% same as foo_fractal_boxcnt.m; computed on x,y coordinates rather than on
% a BW image
%
%~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
function Dvec = FractalDim_8vec( x,y ),

a = zeros((max(y)-min(y)),(max(x)-min(x)));
nx = length(x);
for ii = 1:nx, a(y(ii),x(ii)) = 255; end % ii

% Here foo is like an image that has been already read. It could go
% directly into FractalDim
a = 255 - a; a = uint8(a);
